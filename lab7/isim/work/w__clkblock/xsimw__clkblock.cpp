...
va_list vap)
{
    extern HSim__s6 * createworkMw__clkblock(const char*);
    HSim__s6 *blk = createworkMw__clkblock(label); 
    return blk;
}
...
{
    extern HSim__s6* createWork_clkblock_behavioral(const char*);
    HSim__s6 *blk = createWork_clkblock_behavioral(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("w_clkblock", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("glbl", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("clkblock/behavioral", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("clkblock", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMw__clkblock(const char*);
        topvl = (HSim__s5*)createworkMw__clkblock("w_clkblock");
