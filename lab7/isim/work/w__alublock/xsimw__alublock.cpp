...
va_list vap)
{
    extern HSim__s6 * createworkMw__alublock(const char*);
    HSim__s6 *blk = createworkMw__alublock(label); 
    return blk;
}
...
{
    extern HSim__s6* createWork_alublock_behavioral(const char*);
    HSim__s6 *blk = createWork_alublock_behavioral(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("w_alublock", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("glbl", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("alublock/behavioral", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("alublock", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMw__alublock(const char*);
        topvl = (HSim__s5*)createworkMw__alublock("w_alublock");
